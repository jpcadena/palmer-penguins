...
def find_missing_values(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing_values.any():
        ...
        dataframe['sex'] = dataframe['sex'].fillna(
            dataframe['sex'].mode()[0])
        dataframe['bill_length_mm'] = dataframe['bill_length_mm'].fillna(
            dataframe['bill_length_mm'].median())
        dataframe['bill_depth_mm'] = dataframe['bill_depth_mm'].fillna(
            dataframe['bill_depth_mm'].median())
        dataframe['flipper_length_mm'] = dataframe[
            'flipper_length_mm'].fillna(
            dataframe['flipper_length_mm'].median())
        dataframe['body_mass_g'] = dataframe['body_mass_g'].fillna(
            dataframe['body_mass_g'].median())
    return dataframe
